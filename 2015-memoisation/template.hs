...
updateVar (i, v) s
  | null l    = (i, (Local, v)) : s
  | otherwise = (i, (s', v)) : filter (/= b) s
  where l              = filter ((== i) . fst) s
        b@(_, (s', _)) = head l
...
eval (Var i) _ s
  = getValue i s
eval (Cond c e e') fs s
  | eval c fs s == I 1 = eval e fs s
  | otherwise          = eval e' fs s
eval (OpApp o e e') fs s
  = applyOp o (eval e fs s) (eval e' fs s)
eval (FunApp i es) fs s
  = eval e fs (bindArgs as (evalArgs es fs s) ++ s)
  where (as, e) = lookUp i fs
...
executeStatement (AssignA i e e') fs _ s
  = updateVar (i, assignArray (getValue i s) (eval e fs s) (eval e' fs s)) s
...
executeStatement (While e b) fs ps s
  ...
  | otherwise          = s
executeStatement (Call "" i' es) fs ps s
  = getLocals s ++ getGlobals (executeBlock b fs ps (bindArgs as (evalArgs es fs s) ++ getGlobals s))
  where (as, b) = lookUp i' ps
executeStatement (Call i i' es) fs ps s
  = getLocals s ++ ((i, (Local, snd $ lookUp "$res" s')) : getGlobals s')
  where s' = executeBlock b fs ps (bindArgs as (evalArgs es fs s) ++ getGlobals s)
        (as, b) = lookUp i' ps
...
executeBlock [] fs ps s
  = s
executeBlock (x:xs) fs ps s
  = executeBlock xs fs ps (executeStatement x fs ps s)
...
translate' (Cond c e e') m ids
  = ([If c (b ++ [Assign id ex]) (b' ++ [Assign id ex'])], Var id, is')
  where (b, ex, is) = translate' e m ids
        (b', ex', id:is') = translate' e' m is
translate' (FunApp i es) m (id:ids)
  = ([Call id (lookUp i m) es], Var id, ids)
translate' (OpApp o e e') m ids
  = (b ++ b', OpApp o ex ex', is')
  where (b, ex, is) = translate' e m ids
        (b', ex', is') = translate' e' m is
translate' e m is
  = ([], e, is)
